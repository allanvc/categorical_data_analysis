...
RegressionPlots <- function(fit, range.plt5){
  ...
  plt4 <- regMat %>% 
    ...
    plotly::layout(title = "Standardized Residuals vs Theoretical Quantiles") # usar background branco
  ...
  plt5 <- regMat %>% 
    ...
    plotly::add_trace(x = range.plt5, y= range.plt5, mode = "lines", name="abline") %>% 
...
plotly::plot_ly(x = ~x, y = ~y, name = 'fitted', type = 'scatter', mode = 'lines') %>%
  plotly::add_trace(x= ~temp, y = ~resp, name = 'resp', mode = 'markers', data=challenger) %>%
